        ...
        <main className="min-h-screen w-full flex flex-col items-center pt-20 xs:pt-24 md:pt-32 px-3 xs:px-6 font-grunge selection:bg-primary selection:text-black">
          <div className="w-full max-w-5xl">
            ...
            letterSpacing: "0.06em"
              ...
              <UVText text="Celles et ceux qui bâtissent la légende MTNR. Un collectif, des styles, une vision crue et toujours underground." hiddenText="MTNR - L'ÉLITE DE L'UNDERGROUND - NOUS SOMMES LES FANTÔMES DE LA SCÈNE - LA VIBE QUI RÉSISTE" uvColor="#9b87f5" textSize="text-base xs:text-lg md:text-xl" position="default" />
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5 xs:gap-7 w-full pb-16">
              {artists.map((a, index) => <div key={a.name} className="bg-black/80 paper-texture border-2 border-yellow-400/25 rounded-xl shadow-2xl flex flex-col items-center p-4 xs:p-6 transition-transform hover:scale-105 hover:shadow-yellow-400/20 hover:shadow-xl relative">
                  <div className="w-full overflow-hidden rounded-lg shadow-lg">
                    <img src={index === 1 && isTorchActive && !uvMode ? "/lovable-uploads/anonymous.png" : a.img} alt={a.name} className="w-full aspect-square object-cover rounded-lg grayscale hover:grayscale-0 transition-all duration-300 scale-100 hover:scale-105" />
                  </div>
                  <div className="mt-4 font-black text-lg xs:text-xl text-yellow-400 uppercase text-center tracking-wide">
                    {index === 1 && isTorchActive ? <span className="uv-electric-text animate-[electricText_3s_infinite]">
                        {a.displayName || a.name}
                      </span> : a.name}
                  </div>
                  <div className="text-xs xs:text-sm text-gray-300 italic mt-2 text-center">
                    <UVText text={a.desc} hiddenText={`CODE SECRET: ${a.secretCode} • ACCÈS NIVEAU OMEGA • ARCHIVES CONFIDENTIELLES`} uvColor="#D946EF" textSize="text-xs xs:text-sm" opacity={0.01} />
                  </div>
                </div>)}
            </div>
          </div>
