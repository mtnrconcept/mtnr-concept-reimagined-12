...
export default function Home() {
  return (
    <ParallaxBg>
      <Navbar />
      <main className="relative min-h-screen w-full flex flex-col items-center font-grunge z-10 selection:bg-primary selection:text-black pt-24 xs:pt-28 md:pt-36">
        <section className="w-full flex flex-col items-center justify-center pb-14 px-1 sm:px-3 section-content">
          <div className="inline-block px-4 sm:px-7 py-9 sm:py-12 grunge-border shadow-2xl bg-black/70 max-w-2xl backdrop-blur-lg animate-wiggle mt-5" >
        ...
        <section className="section-content grid grid-cols-1 md:grid-cols-3 gap-7 md:gap-10 mt-7 md:mt-11">
          <div className="rounded-3xl grunge-border px-5 xs:px-7 py-8 shadow-2xl flex flex-col items-center hover-scale bg-black/85 mb-4 md:mb-0">
            ...
            <Link to="/book" className="mt-4 px-6 py-2 bg-yellow-400/90 text-black font-black uppercase tracking-widest rounded-full border border-yellow-900/80 shadow drop-shadow-md hover:bg-yellow-300 hover-scale">
              Book ta session
            </Link>
          </div>
          <div className="rounded-3xl grunge-border px-5 xs:px-7 py-8 shadow-2xl flex flex-col items-center hover-scale bg-black/85 mb-4 md:mb-0">
          ...
          <div className="rounded-3xl grunge-border px-5 xs:px-7 py-8 shadow-2xl flex flex-col items-center hover-scale bg-black/85">
        ...
        <section className="w-full mt-14 xs:mt-16 z-20 section-content">
          ...
          <div className="grid grid-cols-2 xs:grid-cols-3 md:grid-cols-4 gap-5 xs:gap-7">
            {artistImages.map((artist) => (
              <div key={artist.name} className="rounded-xl overflow-hidden bg-black/90 border border-yellow-400/40 shadow-2xl flex flex-col items-center pb-2 hover-scale transition">
